...
export default function ModelingInputPanel({ projectId, onChanged }: ModelingInputPanelProps) {
    ...
    <section className="rounded-lg border border-gray-200 p-4 dark:border-gray-800" aria-labelledby="modeling-inputs-title">
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        <label className="text-sm text-gray-700 dark:text-gray-200">
          ...
          <input type="file" accept=".pdf,.md,.txt" onChange={(event) => setProblem(event.target.files?.[0] || null)} className="block w-full text-xs" />
          <button type="button" disabled={!problem || busy !== null} onClick={() => void upload('problem', problem)} className="mt-2 inline-flex items-center gap-1 rounded bg-blue-600 px-3 py-2 text-xs font-medium text-white disabled:bg-gray-300">
        ...
        <label className="text-sm text-gray-700 dark:text-gray-200">
          ...
          <input type="file" accept=".csv" onChange={(event) => setData(event.target.files?.[0] || null)} className="block w-full text-xs" />
          <button type="button" disabled={!data || busy !== null} onClick={() => void upload('data', data)} className="mt-2 inline-flex items-center gap-1 rounded bg-blue-600 px-3 py-2 text-xs font-medium text-white disabled:bg-gray-300">
