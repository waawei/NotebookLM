...
export default function DeliveryChecklist({ projectId, onChanged }: { projectId: string; onChanged: () => void | Promise<void> }) {
  ...
  useEffect(() => { void modelingApi.checkDeliverables(projectId).then(setCheck) }, [projectId])
  const build = async () => { setBusy(true); try { await modelingApi.buildDeliverables(projectId); await onChanged(); setCheck(await modelingApi.checkDeliverables(projectId)) } finally { setBusy(false) } }
