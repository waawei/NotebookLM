...
export default function FinalPaperApprovalCard({ projectId, review, approval, canDecide = true, onChanged }: { projectId: string; review?: PaperReview; approval?: ApprovalRequest; canDecide?: boolean; onChanged: () => void | Promise<void> }) {
  ...
  return <section className="rounded border border-amber-200 bg-amber-50 p-3 dark:border-amber-900 dark:bg-amber-950"><h2 className="text-sm font-semibold">Final paper approval</h2><button type="button" disabled={blocked || busy || !canDecide} onClick={() => void (approval ? approve() : request())} className="mt-3 rounded bg-green-700 px-3 py-2 text-sm text-white disabled:bg-gray-400">{approval ? 'Approve final paper' : 'Request final approval'}</button></section>
}
